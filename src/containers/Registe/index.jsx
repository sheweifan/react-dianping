...
class Registe extends Component {
    ...
    getVerifyCode() {
      const { phoneError, phonenum } = this.state;
        ...
        .then((data) => {
          ...
          if (data.isOk) {
            ...
            this.timer = setInterval(() => {
              const now = this.state.verifyCodeTimeout - 1;
              if (now === -1) {
                return;
              }
              this.setState({
    ...
    handleSubmit() {
      const { phonenum, password, passwordRepeat, verifyCode } = this.state;
      ...
      postData(registeUrl, {
        ...
        .then((data) => {
          const { isOk, registerSuccess } = data;
          Toast.hide();
          if (isOk) {
            if (registerSuccess) {
